            ...
            <div class="page-breadcrumb">
                <div class="row">
                    <div class="col-12 d-flex no-block align-items-center">
                        <h4 class="page-title">Edit Comment On Post</h4>
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <form action="{{route('comment.update',['id'=>$comment->id])}}"  method="POST" class="form-horizontal">

                                    @csrf

                                    <input type="hidden" name="postold_id" value="{{$comment->post_id}}">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-9">
                                            <select id="post_id" name="post_id" class="form-control @error('post_id') is-invalid @enderror" disabled>
                                                ...
                                                ?>
                                                @foreach($posts as $p)
                                                <option value="{{$p->id}}" {{$post->id==$p->id?'selected':''}} class='parent-{{ $p->cat_id }} post'>{{ $p->title }}</option>
                                                @endforeach
                                            </select>
                                            @error('post_id')
                                                <span class="text-danger"> {{$message}}  </span>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-9">
                                            <textarea id="basic-example"  name="comment"  class="form-control @error('comment') is-invalid @enderror">{{$comment->comment}}</textarea>
                                            @error('comment')
                                                <span class="text-danger"> {{$message}}  </span>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-9">
                                            <select name="commentstatus" class="form-control @error('commentstatus') is-invalid @enderror">
                                                <option value="unpublish" {{$comment->commentstatus=='unpublish'? 'selected':''}}>Un-publish</option>
                                                <option value="publish" {{$comment->commentstatus=='publish'? 'selected':''}}>Publish</option>
                                            </select>
                                            @error('commentstatus')
                                                <span class="text-danger"> {{$message}}  </span>
                                            @enderror
